...
<form action="{{ route('warehouses.update', $warehouse) }}" method="POST" class="mt-3" id="warehouseForm">
    @csrf
    @method('PUT')
    ...
    <div class="mb-3" id="wh-order">
        ...
        <input type="number" name="sort_order" class="form-control" value="{{ $warehouse->sort_order }}">
    </div>
    <div class="form-check mb-3" id="wh-active">
        <input type="checkbox" name="is_active" class="form-check-input" id="is_active" {{ $warehouse->is_active ? 'checked' : '' }} value="1">
